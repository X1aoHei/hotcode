...
export const useProgressStore = defineStore('progress', () => {
  ...
  async function init() {
    try {
      const [progressData, wrongIds] = await Promise.all([
        api.getProgress(),
        api.getWrongSet(),
      ])
      const s: Record<number, MasteryStatus> = {}
      const lv: Record<number, number> = {}
      for (const [idStr, rec] of Object.entries(progressData)) {
  ...
  /** 切换错题标记 */
  async function toggleWrong(id: number) {
    ...
    try {
      await api.toggleWrong(id)
    } catch (e) {
      // 回滚
      if (wasIn) {
        wrongSet.value.push(id)
      } else {
        wrongSet.value = wrongSet.value.filter((wid) => wid !== id)
      }
      ElMessage.error('更新错题集失败')
      console.error(e)
    }
  }
  ...
  return { status, lastViewed, wrongSet, init, setStatus, getStatus, markViewed, reset, toggleWrong, isWrong, stats }
})
